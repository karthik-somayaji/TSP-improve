...
class Lee:
    ...
    def route_all(self, lst, act_lst, alg='astar', check=False):
        ...
        return self.total_wire_length() if all_ok else None
...
class router():
    def __init__(self, n, m, type_fn):

        if(type_fn=='Lee_15' or type_fn=='Lee_22' or type_fn=='Lee_29'):
            n = m = int(type_fn[4:])
            factor = (n+7) // 8
        ...
        for i in range(num_nets):
            l = self.coords[str(i)]
            t = [(x*mult,y*mult) for (x,y) in l]
            self.coords[str(i)] = t
        self.lst = [(x, (a*mult, b*mult), (c*mult, d*mult)) for (x,(a,b), (c,d)) in self.lst]
       

    def calc_cost(self, i, routed_lst):
        act_lst = [(str(i), self.coords[str(i)][0], self.coords[str(i)][1]) for i in routed_lst]
        cost = self.a.route_all(self.lst, act_lst, alg=self.alg )
        return cost
    ...
    def examples(self, type_fn):
        ...
        coords_custom_8_4_20 = {}
        temp = [ {a1:[(a2,a3), (a4,a5)]} for (a1, (a2,a3), (a4,a5)) in lst_custom_8_4_20]
        for d in temp:
            coords_custom_8_4_20.update(d)
        

        if('custom_8_4_20' in type_fn):
            self.coords = coords_custom_8_4_20
            self.lst = lst_custom_8_4_20
        elif('syn_4_20' in type_fn):
            self.coords = coords_syn_4_20
            self.lst = lst_syn_4_20
        elif('syn_4_12' in type_fn):
            self.coords = coords_syn_4_12
            self.lst = lst_syn_4_12
            ...
            self.coords = coords_syn_4_24
            self.lst = lst_syn_4_24
        elif('syn_4_28' in type_fn):
            self.coords = coords_syn_4_28
            self.lst = lst_syn_4_28
        elif('syn_4_32' in type_fn):
            self.coords = coords_syn_4_32
            self.lst = lst_syn_4_32
        elif('Lee' in type_fn):
            self.coords = coords
            self.lst = lst
