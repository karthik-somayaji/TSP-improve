...
class transformer_inst():
    def __init__(self, problem,
        ...
        elif('random' in self.prob_type):
            self.n= int(self.prob_type[7:9])
            self.m= int(self.prob_type[10:12])  
    ...
    def save_model(self, prob_name, trial):
        results_dir = "saved_models"
        if not os.path.exists(results_dir):
            os.makedirs(results_dir)
        path = "{}/{}_transformer_inst_{}".format(results_dir, prob_name, trial)
